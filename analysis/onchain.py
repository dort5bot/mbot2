...
class OnchainAnalyzer:
    ...
    def __init__(self, binance_api: Optional[BinanceAPI] = None, 
        ...
        """
        # Singleton pattern için kontrol - sadece bir kez initialize et
        if not self._initialized:
            self.binance = binance_api
            # Config yönetimi - önce parametre, sonra global config'ten onchain bölümü
            if config:
                self.config = config
            else:
                # Global config'ten onchain bölümünü al
    ...
    async def _ensure_session(self) -> None:
        ...
        if self.session is None or self.session.closed:
            self.session = aiohttp.ClientSession(
                timeout=aiohttp.ClientTimeout(total=self.config.API_TIMEOUTS["glassnode"])
            )
